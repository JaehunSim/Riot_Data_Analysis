...
def conversion(data,gameTypes):
    ...
    w2 = pd.Series(gameDataDict3, copy=True)
    
    for index,value in enumerate(w2):
        try:
            w2[index]=transfromDictToList(value)
        except:
            pass
    ...
    w2 = w2.drop(dropList)
    for index,value in enumerate(w2):
        value.index = data.columns         
    w2 = w2.apply(pd.Series.to_frame)
    w2 = w2.apply(pd.DataFrame.transpose)
    w4 = pd.concat(w2.values)
    w4.index = range(len(w4))
    w4.to_csv(PATH+"DB_file\\temp\\"+gameTypes+"_gameDataTable.csv", index=None, encoding="euc-kr")    
    print("end")
    
def main(fileName, gameTypes):
    with open(PATH + "code\\"+fileName, "r") as f:
        gameDataDict = f.readlines()
    conversion(gameDataDict,gameTypes)

#fileNameList = ["EUW1_RANKED_FLEX_SR_data.txt","EUW1_RANKED_SOLO_5x5_data.txt","KR_RANKED_SOLO_5x5_data.txt","NA1_RANKED_SOLO_5x5_data.txt","KR_RANKED_FLEX_SR_data.txt"]
#gameTypesList = ["EUW1_FLEX","EUW1_SOLO","KR_SOLO","NA1_SOLO","KR_FLEX"]
#
#for index in range(len(fileNameList)):
#    main(fileNameList[index],gameTypesList[index])
